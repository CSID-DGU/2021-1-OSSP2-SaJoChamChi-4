...
class ProfilePresenter extends Component {
  ...
  render() {
    return this.state.data.map((data) => (
      <View>
        <Text style={{ fontSize: 20 }}>이름 : {data.usr_Name}</Text>
        <Text style={{ fontSize: 20 }}>닉네임 : {data.usr_Nickname}</Text>
        <Text style={{ fontSize: 20 }}>Email : {data.usr_Email}</Text>
        <Text style={{ fontSize: 20 }}>생일 : {data.usr_Birth}</Text>
        <Text style={{ fontSize: 20 }}>주소 : {data.usr_Address}</Text>
        <Text style={{ fontSize: 20 }}>ID : {data.user_Id}</Text>
