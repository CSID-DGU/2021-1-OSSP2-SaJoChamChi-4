...
class RefriListPresenter extends Component {
  ...
  // 삭제 임시 저장
  //  delItem = (item) => {
  //     fetch('http://192.168.0.190:3344/refri/delete',{
  //         method: "post",
  //         headers :{
  //             "content-Type" : "application/json",
  //         },
  ...
  render() {
    ...
    return this.state.data.map((data) => (
      <View style={{ flexDirection: "row", width: "100%" }}>
        <Text style={{ fontSize: 15, width: "25%", textAlign: "center" }}>
          {data.rf_Pname}
        </Text>
        <Text style={{ fontSize: 15, width: "15%", textAlign: "center" }}>
          {data.rf_Number}
        </Text>
        <Text style={{ fontSize: 15, width: "25%", textAlign: "center" }}>
          {this.ToDate(data.rf_Indate)}
        </Text>
