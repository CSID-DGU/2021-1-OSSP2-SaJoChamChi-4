...
const InsertItem = ({ route, navigation }) => {
  ...
  const _handleSignupButtonPress = async () => {
    try {
      ...
      alert("Success!\n추가되었습니다!");
      navigation.dispatch(
        CommonActions.reset({ index: 0, routes: [{ name: "MyRefri" }] })
      );
    }
  };
  ...
  return (
    <KeyboardAwareScrollView extraScrollHeight={20}>
      <Container>
        ...
        <RNPickerSelect
          onValueChange={(value) => setFrozen(value)}
          placeholder={{
            label: "냉동/냉장여부",
            value: null,
          }}
        ...
        <RNPickerSelect
          onValueChange={(value) => setFkind(value)}
          placeholder={{
            label: "음식분류를 선택해주세요",
            value: null,
          }}
