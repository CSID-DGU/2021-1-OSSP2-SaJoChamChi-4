...
class MyGoodBoardPresenter extends Component {
  ...
  click = (user) => {
    fetch("http://192.168.0.190:3344/board/MygoodBoard", {
      method: "post",
      headers: {
  ...
  onPressHandle = async (data) => {
    ...
    if (this.state.data2.length == 0) {
      this.props.navigation.navigate("Board", {
        screen: "DetailView",
        params: { data: data, data2: null },
      });
    } else
      this.props.navigation.navigate("Board", {
        screen: "DetailView",
        params: { data: data, data2: this.state.data2 },
      });
  };
  ...
  getGoodInfo = async (data) => {
    ...
    res = await fetch("http://192.168.0.190:3344/good/IsGood", {
      method: "post",
      headers: {
        "content-Type": "application/json",
      },
