...
click =  ()  => {
  ...
  fetch('http://192.168.0.184:3344/test/getUser',{
    headers: {
...
click2 = ()  => {
    this.setState(({clicked2: true}));
     fetch('http://192.168.0.184:3344/login/Login',{
        method: "post",
        headers :{
            "content-Type" : "application/json",
        },
...
renderCategories2(){
    console.log("data2");
    console.log(this.state.data2);
    return this.state.data2.map((data)=> <View><Text>UserId : {data.usr_Id}</Text>
    <Text>Name : {data.usr_Name}</Text>
    </View>)
}


    unclick =() => {this.setState(({clicked: false, clicked2: false})); console.log(this.state.data, this.state.clicked, this.state.clicked2)};
    ...
    render(){
        return(
            <Container >
                ...
                {
                    !this.state.clicked2 ? <Button title="get2" onPress={this.click2} /> : this.renderCategories2()
                }
                {
